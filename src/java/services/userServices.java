        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(request.getParameter("mailOrUserName")!=null){
                ...
                if(request.getParameter("pt_user")!=null){
                    data.put("status", new userControl().mailOrUserNameValidate(request.getParameter("pt_user")));
                    out.print(data);
                }                
            }
            if(request.getParameter("mailValidate")!=null){
                ...
                if(request.getParameter("pt_mail")!=null){
                    data.put("status", new userControl().mailValidate(request.getParameter("pt_mail")));
                    out.print(data);
                }                
            }
            if(request.getParameter("userValidate")!=null){
                JSONObject data = new JSONObject();
                if(request.getParameter("pt_user")!=null){
                    data.put("status", new userControl().userValidate(request.getParameter("pt_user")));
                    out.print(data);
                }                
            }
            if(request.getParameter("action")!=null && request.getParameter("action").equals("insert")){
                ...
                obj.setFl_password(request.getParameter("user_password"));
                data.put("status", new userControl().addUser(obj));
                data.put("sendMailStatus", new mailControl().sendMailActiveAcount(obj));
                aes sec = new aes();
                sec.addKey("2015");
                String liga = "userName"+"="+request.getParameter("user_name");
                data.put("link", "ac="+sec.encriptar(liga));
            ...
            if(request.getParameter("action")!=null && request.getParameter("action").equals("requestPassword")){
                JSONObject data = new JSONObject();
                userModel obj = new userModel();                
                ArrayList<userModel> list=new userControl().selectUser(request.getParameter("user_name"));
                for (userModel list1 : list) {
                ...
                data.put("sendMailStatus", new mailControl().sendMailResetPassword(obj));
                out.print(data);
            }
            if(request.getParameter("action")!=null && request.getParameter("action").equals("changePassword")){
                JSONObject data = new JSONObject();
                data.put("status", new userControl().changePasswordUser(request.getParameter("user_name"), request.getParameter("new_password")));
                out.print(data);
            }
        }
    }
